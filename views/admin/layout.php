...
<?php
// Ensure helpers are available for CSRF + validation
$helpersPath = __DIR__ . '/../../helpers';
if (is_file($helpersPath . '/Validation.php')) {
    include_once $helpersPath . '/Validation.php';
}

// Simple flash display (session-based)
if (session_status() === PHP_SESSION_NONE) {
    session_start();
}
if (!empty($_SESSION['_flash'])) {
    echo '<div style="position:fixed;right:18px;top:72px;z-index:1200;">';
    foreach ((array)$_SESSION['_flash'] as $f) {
        $type = $f['type'] ?? 'info';
        $msg  = htmlspecialchars($f['message'] ?? '');
        $bg = $type === 'success' ? '#16a34a' : ($type === 'error' ? '#dc2626' : '#0ea5e9');
        echo "<div style=\"background:{$bg};color:#fff;padding:10px 14px;border-radius:8px;margin-bottom:8px;box-shadow:0 8px 24px rgba(2,6,23,0.4);\">" . $msg . '</div>';
    }
    echo '</div>';
